	...
	$('#bt_proses').on('click keypress',function() {
		if ($('#total_beli').val().replace(/[^-.\d]/g, '') != 0) {
			...
			alert('Data Masih belum diisi dan total tidak boleh 0')
		}
	});
		...
		$('#myModalProses').modalSteps({
    	  ...
    	  completeCallback:function() {
			if ($('.d_Pemasok').val()) {
				...
				alert('Pemasok Masih Belum Diisi');
			}
    	  },
    	  callbacks: {},
...
//isi data barang dengan modal
	for (let b=0;b<$(".sum_barang").val(); b++) {
		$(".chs_barang"+b).click(function(){
			...
			$("#kd_barang").val($('.no_barang'+b).val());
			$("#nm_barang").val($('.nama_barang'+b).val());
			$("#bunit_barang").val($('.b_unit'+b).val());
			$("#unit_barang").val($('.n_unit'+b).val());
			$("#unit_conv").val($('.konversi_unit'+b).val());
			$("#hpp_barang").val($('.harga_pokok'+b).val());
			$("#h_barang").val($('.harga_beli'+b).val());
			$("#akun_barang").val($('.b_perkiraan'+b).val());
