...
$('#bt_proses').on('click keypress', function () {
	if ($('#total_beli').val().replace(/[^-.\d]/g, '') != 0) {
		...
		Swal.fire('Retur Pembelian', 'Data masih belum diisi dan total tidak boleh 0', 'warning')
	}
});
//modalproses
			...
			if ($('.d_Pemasok').val()) {
				if ($('#sisa_bayar').val() == 0 || $('#sisa_bayar').val() == 0.00) {
					for (let br = 0; br < sel; br++) {
						parseFloat($('#h_barang' + br).val($('#harga_barang' + br).val().replace(/[^-.\d]/g, '')));
					...
					Swal.fire('Retur Pembelian', 'Transaksi retur barang tidak dapat melalui pembayaran kredit', 'warning');
				}

			} else {
				Swal.fire('Retur Pembelian', 'Data Pemasok Belum diisi', 'warning')
				$('.d_Pemasok').focus()
			}
		},
		callbacks: {},
...
for (let s = 0; s < $(".sum_pemasok").val(); s++) {
	$(".chs_pemasok" + s).on('click keypress', function () {
		...
		$("#ketr").val("Retur Pembelian a/n " + $('.nama_pemasok' + s).val())
	});
}
...
$("#add_barang").on('click keypress', function () {
	if ($('#nm_barang').val()) {
		...
		$('#dynamic_field').append('<tr id="row' + u + '" "class="dynamic-added">' +
			...
			'</td>' +
			'<td class="p-2" style="width:16%;">' +
			'<input type="text" class="form-control m-0 p-2 bg-white col totalsPb text-right" id="total' + u + '" name="total' + u + '" data-index="' + u + '" value="' + number_format($("#h_barang").val(), 2, '.', ',') + '">' +
			'</td>' +
		...
		Swal.fire('Retur Pembelian', 'Barang masih kosong', 'warning')
	}
});
...
$(".ins_retur").click(function () {
	var detail = $("#no_pembelian").val();
	$('#reff').val(detail)
	$('#myModalPembelian').modal('hide');
	$('#row0').remove();
	$.getJSON("<?= site_url('Pembelian/getPembelian?n_pembelian=') ?>" + detail, function (json) {
		var u;
		for (u = 0; u < json.length; u++) {
			...
			$('#dynamic_field').append('<tr id="row' + u + '" "class="dynamic-added">' +
				...
				'<input type="hidden" name="perkiraan' + u + '" id="perkiraan' + u + '" value="' + json[u].akun + '" readonly>' +
		...
		$('.d_Pemasok').val($('#dt_pemasok').val());
		$('#reff_pembelian').val($('#reff_pemb').val());
		$('#ketr_pembelian').val($('#ketr_pemb').val());
		$('.sum_baris').val(parseInt(json.length));
		$("#ketr").val('Retur Pembelian dari ' + $("#no_pembelian").val())
		$("#dr_pembelian").val($("#no_pembelian").val());
		hitungTotal();
	});
	var n_pemasok = $('#dt_pemasok').val().split(" | ");
	$.getJSON("<?= site_url('Pemasok/getPemasok?n_pemasok=') ?>" + n_pemasok[0], function (json) {
		console.log(json);
		...
		if (json != false) {
			getHutang(n_pemasok[0]);
			$('#n_barang0').focus();
		}
	});
});

$(".detail_pembelian").click(function () {
	var detail = $(this).data('trans');
	$("#no_pembelian").val($(this).data('trans'));
	$("#dt_pemasok").val($(this).data('pemasok'));
	$("#reff_pemb").val($(this).data('reff'));
	$("#ketr_pemb").val($(this).data('ketr'));
	...
	$.getJSON("<?= site_url('Pembelian/getPembelian?n_pembelian=') ?>" + detail, function (json) {
		...
		for (i = 0; i < json.length; i++) {
			html += '<tr>' +
				...
				'</tr>';
			$('#totalPembelian').val(number_format(json[i].totalPemb, 2, '.', ','));
		}
		$('#show_detail').html(html);
	});
});
...
$(document).ready(function () {
	...
	$('#myModalPembelian').on('hidden.bs.modal', function () {
		$("#bt_proses").focus();
	});
});
