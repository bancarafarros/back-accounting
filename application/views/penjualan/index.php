<form action="<?= site_url('penjualan/do_penjualan') ?>" id="formPenj" method="POST">
	<div class="col justify-content-md-center p-0">
		<div class="row justify-content-md-center">
			...
			<div class="container col-md-12 col-lg-12 col-xlg-12 col-12">
				<div class="row justify-content-md-center">
					<div class="col-lg-12">
						<h6 class="card-title m-b-0 p-2 text-light bg-success">Detail Pejualan</h6>
								...
								<div class="row">
									...
									<div class="row col-xl-3 col-md-4 col-sm-4 bg-danger">
										<label class="col-sm-7 col-6 col-md-5 control-label col-form-label pl-0 pr-0 text-light">
											<h5><b class="text-light"><i>TOTAL :</i></b></h5>
										</label>
										<div class="col-sm-6 col-6 col-md-7">
											<input type="text" class="form-control text-right border-0 bg-danger text-light pl-0 pr-0" style="font-size:16px;" id="total_jual" name="totalJualbrg" value="0.00" readonly>
										</div>
									</div>
								</div>
								<div class="row">
							...
							<tbody>
								...
								foreach ($d_pelanggan as $key => $value) { ?>
									<tr>
										...
										<td align="right"><button type="button" class="btn btn-sm btn-success chs_pel chs_pelanggan<?= $no ?>" data-dismiss="modal"><i class="fa fa-check"></i></button></td>
									</tr>
								<?php $no++;
								} ?>
							...
							<tbody>
								...
								foreach ($d_pelanggan as $key => $value) { ?>
									<tr>
										...
										<input type="hidden" class="nama_sales<?= $no ?>" value="<?= $value->nama_sales ?>">
										<input type="hidden" class="n_sales<?= $no ?>" value="<?= $value->n_sales ?>">
										<td align="right"><button type="button" class="btn btn-sm btn-success chs_sales<?= $no ?>" data-dismiss="modal"><i class="fa fa-check"></i></button></td>
									</tr>
								<?php $no++;
								} ?>
							...
							<tbody>
								...
								foreach ($d_bank as $key => $value) { ?>
									<tr>
										...
										<td align="center">
											<button type="button" class="btn btn-sm btn-success chs_bnk chs_bank<?= $no ?>" data-dismiss="modal"><i class="fa fa-check"></i></button>
										</td>
									</tr>
								<?php $no++;
								} ?>
	...
	<div class="modal fade" id="myModalProses" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true" style="z-index:1045;overflow-y:auto;" aria-labelledby="myModalLabel">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row hide" data-step="1" data-title="Detail Database">
						<div class="container">
							<div class="form-group m-0">
								<label class="col-6 col-sm-6 col-md-6 p-0">Pelanggan</label>
								<label class="col-6 col-sm-4 col-md-4 p-0">Sales</label>
							</div>
							<div class="form-group form-row mb-3">
								...
								<div class="input-group col-12 col-sm-12 col-md-6 p-1">
									<input type="text" class="form-control d_Sales" name="n_sales" value="" required>
									<div class="input-group-append">
										<a href="#myModalSales" data-toggle="modal">
											<button class="btn btn-success" type="button"><i class="fa fa-search"></i></button>
										</a>
									</div>
									<input type="hidden" class="form-control d_nSales" name="n_sales" value="" readonly>
								</div>
							</div>
							<div class="form-group m-0">
								<label class="col-6 col-sm-6 col-md-6 p-0">Refferensi</label>
								<label class="col-6 col-sm-4 col-md-4 p-0">Catatan</label>
							</div>
							<div class="form-group form-row mb-3">
								<div class="input-group col-11 col-sm-12 col-md-6 p-1">
							...
							<div class="form-group m-0">
								<label class="col-6 col-sm-6 col-md-6 p-0">Tanggal Transaksi</label>
								<label class="col-6 col-sm-4 col-md-4 p-0">Tanggal Jatuh Tempo</label>
							</div>
							<div class="form-group form-row m-0 pb-3 mb-3 border-bottom border-success">
								<div class="input-group col-11 col-sm-12 col-md-6 p-1">
									<input type="text" class="form-control datepicker tanggal" id="datepicker-autoclose1" name="tanggal" value="<?= date("Y-m-d") ?>">
									<div class="input-group-append">
										<span class="input-group-text"><i class="fa fa-calendar"></i></span>
									</div>
								</div>
								<div class="input-group col-11 col-sm-12 col-md-6 p-1">
									<input type="text" class="form-control datepicker jatuh_tempo" name="jatuh_tempo" id="datepicker-autoclose2" value="<?= date("Y-m-d") ?>" placeholder="mm/dd/yyyy">
									<div class="input-group-append">
										<span class="input-group-text"><i class="fa fa-calendar"></i></span>
									</div>
								</div>
							...
							<div class="form-group m-0">
								<label class="col-6 col-sm-6 col-md-6 p-0">PPN</label>
								<label class="col-6 col-sm-4 col-md-4 p-0">Biaya Kirim</label>
							</div>
								...
								<div class="input-group">
									<div class="btn-group justify-content-md-center" role="group" aria-label="Basic example">
										...
										<a href="#myModalBank" data-toggle="modal">
											<button type="button" class="btn btn-defaul" style="min-width:65px; max-width:65px" id="c_bank" value="bank">BANK</button>
										</a>
										<input type="text" class="form-control bank" id="d_Bank" name="no_bank" value="" readonly>
										<div class="input-group-append">
											<a href="#myModalBank" data-toggle="modal">
												<button class="btn btn-success bank" id="bank" type="button"><i class="fa fa-search"></i></button>
											</a>
										</div>
									</div>
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-success js-btn-step proses"  onclick="proses()" data-orientation="next"><i class="fa fa-check"></i> Proses</button>
					</div>
				</div>
			</div>
		</div>
