                        ...
                        <tbody>
                            ...
                            ?>
                                        ...
                                        <tr>
                                            ...
                                            <!-- <td><?= "Rp." . formatRupiah($r->total) ?></td> -->
                                            <td><?= $r->rektor == 1 ? "<span class='text-success'>Disetujui</span>" : "<span class='text-danger'>Belum/Tidak Disetujui</span>"; ?></td>
                                            <td>
                                                <?php if ($r->rektor == 0) : ?>
                                                    <button type="button" data-url="<?= site_url('approvalpencairan/doagree') ?>/<?= $r->id_rab ?>" class="btn btn-primary btn-sm agree"><i class="fa fa-check"></i> Setujui</button>
                                                <?php else : ?>
                                                    <button type="button" data-url="<?= site_url('approvalpencairan/donotagree') ?>/<?= $r->id_rab ?>" class="btn btn-danger btn-sm notagree"><i class="fa fa-times"></i> Batalkan</button>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                            <?php $no++;
...
<script type="text/javascript">
    ...
    $('body').on('click', '.agree', function() {
        ...
        swal({
            title: 'Warning',
            text: 'Yakin untuk menyetujui pencairan Anggaran?',
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then((doAgree) => {
    ...
    $('body').on('click', '.notagree', function() {
        ...
        swal({
            title: 'Warning',
            text: 'Yakin untuk membatalkan persetujuan pencairan Anggaran?',
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then((donotAgree) => {
